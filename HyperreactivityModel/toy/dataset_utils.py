"""
Useful functions for peptide processing.
"""
...
def get_complete_sequence(proteinID: str) -> str:
    """
    Helper to retrieve the UniProt amino acid sequence for a protein.
    """

    currentUrl = "http://www.uniprot.org/uniprot/" + proteinID + ".fasta"
...
def process_dataset(peptides: pd.DataFrame, amino_acid: str="M", amino_acid_str: str="Methionine", analysis_threshold: int=20, modifications: list[str]=["649.3660", "655.3735"]) -> pd.DataFrame:
    """
    Process a standard isoTOP-ABPP dataset to extract peptide locations
    ...
    # Extract left prefix of modified Met site (for subsequent indexing)
    ...
    # Find Met site in full protein sequence
    peptides[f"{amino_acid_str} Location"] = peptides["Peptide Location"] + peptides["Left Prefix Length"]

    # Annotate Met site
    peptides["Site"] = peptides["Protein ID"] + "_M" + peptides[f"{amino_acid_str} Location"].astype(str)
...
def calculate_accessibilities(cif_dir: os.path, pae_dir: os.path, uniprotIDs: list[str], radii: list[int], angles: list[int]=[180], smooth: bool=True) -> pd.DataFrame:
    """
    Calculate the amino acid crowdedness (pPSE) of each Met site at various radii and angles.
    """
